...
PARSER_AGENT_PROMPT = """
    ...
    먼저 입력 텍스트를 행 단위로 분해하고, 각 행에서 담당자 이름, 프로젝트 또는 업무명, 상태(진행/완료/예정/지연), 날짜 정보, 그리고 업무 설명을 추출한다. 이후 규칙 기반으로 데이터를 매핑한다. center_name은 항상 "AX수행N센터"(예: "AX수행2센터") 형태로 고정한다. member_name은 텍스트에서 사람 이름(예: 이용필, 조윤혜)을 우선 탐지하고, 해당 행에 없을 경우 상위 행의 값을 상속하며, 끝까지 찾지 못하면 오류로 처리한다. status는 "진행", "완료", "예정", "지연" 값을 그대로 사용한다.
    ...
    마지막으로 출력 전에 자기 검증을 수행한다. JSON 이외의 텍스트가 포함되어 있는지 확인하고, 포함되어 있다면 제거한다. 모든 record에 필수 필드가 존재하는지 확인한다.
...
writer_agent_prompt = """
  아래 순서대로 보고서 초안을 작성하라.
    ...
    【Step 2 — 마크다운 출력 규칙 (엄수)】
    - write_report_tool이 반환한 markdown_report를 그대로 최종 출력으로 사용한다.
    - 도구 출력을 임의로 재구성·요약·합산하지 않는다.
    - 섹션 1(요약)의 bullet 문장 표현만 자연스럽게 다듬을 수 있다.

    ※ 아래 규칙을 반드시 준수한다:
    - "## 2. 프로젝트별 주간 실적 및 작업 내역" 섹션은 도구가 생성한 구조를 100% 유지한다.
      · 각 프로젝트는 "### N. 프로젝트명" 헤더로 구분된다.
      · 각 프로젝트 테이블은 "| 구분 | 작업 내용 |" 2열 구조다.
      · 프로젝트를 하나의 평면 테이블로 합치거나, 열을 추가(프로젝트명 열 등)하지 않는다.
      · 행을 삭제하거나 작업 내용을 요약·병합하지 않는다.
    - "---" 구분선으로 나뉜 프로젝트 섹션 순서를 바꾸지 않는다.

    【Step 3 — 출력 형식】
    - 최종 출력은 마크다운 본문만 반환한다.
    - PASS/RETRY 같은 판정 텍스트를 출력하지 않는다.

    [섹션 2 출력 예시 — 이 구조를 반드시 유지]
    ...
    | 구분 | 작업 내용 |
    |---|---|
    | 완료 | Main Agent 프레임워크 구현 |
    | 진행 | KPI 지수 관련 마트 및 테이블 구조 분석 |

    > **이슈**: 고객사 협의 필요 목록 확인 후 내부 일정 검토
